...
//This Network.cpp file is the implementation file for designing the Network class' functions and returning the proper information that the program was made for.
...
LinkedList<Post*> Network<ItemType>::bSortByUsername(Comparator compare, int* counter){
    LinkedList<Post*>copy_of_feed(feed_);
    copy_of_feed.bubbleSort(compare, counter);
    return copy_of_feed;
}