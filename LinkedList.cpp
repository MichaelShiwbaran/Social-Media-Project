...
void LinkedList<ItemType>::bubbleSort(Comparator compare, int* counter){
    Node<ItemType> *current_pt = head_;
    for (int i = 0; i < size_; i++){
        current_pt = head_;
        while (current_pt->getNext() != nullptr){
            if(!compare(*(current_pt->getItem()),*(current_pt->getNext()->getItem()))){
                ItemType temp = current_pt->getItem();
                current_pt->setItem(current_pt->getNext()->getItem());
                current_pt->getNext()->setItem(temp);
                (*counter)++;
            }
            current_pt = current_pt->getNext();
        }
    }
}
// function implementation of bubble sort